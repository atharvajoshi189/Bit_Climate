...
// ==============================================================================
//  CITIZEN SCIENCE HUB (FIXED FOR HYDRATION)
// ==============================================================================
    ...
    return (
        <div className="bg-[#161B22] p-8 rounded-2xl shadow-xl max-w-4xl mx-auto border border-gray-700">
            <div className="grid md:grid-cols-2 gap-8 items-center">
                ...
                >
                    <label htmlFor="file-upload" className="sr-only">Upload image for analysis</label>
...
// ==============================================================================
//  ECO-VERIFY AI FACT-CHECKER (FIXED FOR HYDRATION)
// ==============================================================================
const EcoVerify = () => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const response = await fetch('http://127.0.0.1:8000/verify_claim', {
                method: 'POST',
    ...
    return (
        <div className="bg-[#161B22] p-8 rounded-2xl shadow-xl max-w-3xl mx-auto border border-gray-700">
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-4">
                ...
                />
                <button type="submit" disabled={!isClient || isLoading} className="w-full bg-teal-500 text-black font-bold py-3 rounded-lg hover:bg-teal-400 disabled:bg-gray-600 disabled:cursor-not-allowed">
...
// ==============================================================================
//  NEW COMPONENT: CLIMATE IMPACT SIMULATOR
// ==============================================================================
const ClimateImpactSimulator = () => {
    const [tempRise, setTempRise] = useState(1.5);
    const [deforestation, setDeforestation] = useState(20);

    const seaLevelRise = (tempRise * 0.6).toFixed(1);
    const droughtRisk = Math.min(100, (15 + tempRise * 12)).toFixed(0);
    const co2Increase = (deforestation * 0.5).toFixed(1);
    const biodiversityLoss = Math.min(100, (10 + deforestation * 0.8)).toFixed(0);
    ...
    return (
        <div className="bg-[#161B22] p-8 md:p-12 rounded-2xl shadow-xl max-w-5xl mx-auto border border-gray-700">
            <div className="grid md:grid-cols-2 gap-12">
                <div className="space-y-8">
                    <div>
                        <label htmlFor="temp-slider" className="block text-lg font-medium text-white">Global Temperature Rise</label>
                        <p className="text-teal-400 text-3xl font-bold mb-2">+{tempRise.toFixed(1)}°C</p>
                        <input id="temp-slider" type="range" min="0.5" max="5" step="0.1" value={tempRise} onChange={(e) => setTempRise(parseFloat(e.target.value))} className="w-full h-2 bg-gray-700 rounded-lg appearance-none cursor-pointer accent-teal-500" />
                    </div>
                    <div>
                        <label htmlFor="deforest-slider" className="block text-lg font-medium text-white">Annual Deforestation Rate</label>
                        <p className="text-orange-400 text-3xl font-bold mb-2">{deforestation} Mha</p>
                        <input id="deforest-slider" type="range" min="0" max="100" step="1" value={deforestation} onChange={(e) => setDeforestation(parseInt(e.target.value))} className="w-full h-2 bg-gray-700 rounded-lg appearance-none cursor-pointer accent-orange-500" />
                    </div>
                </div>
...
// ==============================================================================
//  MAIN HOME PAGE COMPONENT (UPDATED WITH ALL FEATURES)
// ==============================================================================
export default function HomePage() {
  ...
  useEffect(() => {
    const observer = new IntersectionObserver(([entry]) => { setIsHeroButtonVisible(entry.isIntersecting); }, { threshold: 0.5 });
    const currentRef = heroButtonRef.current;
  ...
  return (
    <>
      <style>{`
          ...
          .parallax-bg { background-image: url('https://t3.ftcdn.net/jpg/09/47/87/50/360_F_947875038_QlOtNAu4VQfWUw3zDwOPAtY3y4DaqBHw.jpg'); background-attachment: fixed; background-position: center; background-repeat: no-repeat; background-size: cover; }
          ...
          .portal-card { width: 100%; aspect-ratio: 9 / 12; background: #000; border-radius: 1.5rem; transition: transform 0.5s cubic-bezier(0.19, 1, 0.22, 1); box-shadow: 0 20px 50px rgba(0,0,0,0.2); position: relative; cursor: pointer; overflow: hidden; border: 1px solid rgba(255, 255, 255, 0.1); }
          .portal-card:hover { transform: translateY(-10px); box-shadow: 0 40px 80px rgba(0,0,0,0.3); }
          .portal-video { position: absolute; top: 0; left: 0; width: 100%; height: 100%; object-fit: cover; transition: filter 0.5s ease-out, transform 0.5s ease-out; }
          .portal-card:hover .portal-video { filter: blur(10px) brightness(0.6); transform: scale(1.1); }
          .portal-content { position: absolute; inset: 0; padding: 1.5rem; display: flex; flex-direction: column; justify-content: center; align-items: center; z-index: 3; color: white; text-align: center; }
          .content-default { transition: opacity 0.4s ease-out, transform 0.4s ease-out; }
          .portal-card:hover .content-default { opacity: 0; transform: translateY(10px); }
          .content-hover { position: absolute; inset: 1.5rem; display: flex; flex-direction: column; justify-content: center; align-items: center; opacity: 0; transform: translateY(20px); transition: opacity 0.5s ease-out 0.1s, transform 0.5s ease-out 0.1s; }
          .portal-card:hover .content-hover { opacity: 1; transform: translateY(0); }
      ...
      <main>
        ...
        <section className="h-screen w-full flex flex-col items-center justify-center relative parallax-bg">
          ...
          <div className="text-center px-4 z-10 flex flex-col items-center animate-fade-in">
            ...
            <p className="mt-8 text-lg md:text-xl text-gray-200 max-w-3xl mx-auto" style={{ animationDelay: '0.2s' }}>
              Pioneering data-driven solutions to monitor, analyze, and combat critical environmental challenges.
            </p>
            <div ref={heroButtonRef} className="mt-20" style={{ animationDelay: '0.4s' }}>
              <EcobotButton />
            </div>
          </div>
        </section>
        ...
        <motion.div ref={contentRef} style={{ backgroundColor, transition: 'background-color 0.3s ease-in-out' }} className="relative">
            ...
            <motion.section className="py-24" variants={sectionAnimation} initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.2 }}>
                <div className="container mx-auto px-6">
                    <motion.div className="text-center max-w-3xl mx-auto mb-16" variants={sectionAnimation}><h2 className="text-4xl font-bold text-white">Become a Citizen Scientist</h2><p className="text-lg text-gray-400 mt-4">See a problem? Report it. Help us crowdsource real-time data by uploading an image of any environmental issue you find. Our AI will analyze and categorize it.</p></motion.div>
                    <CitizenScienceHub />
                </div>
            </motion.section>

            {/* --- Your 3 Cards Section --- */}
            <motion.section id="services" className="py-24" variants={sectionAnimation} initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.2 }}>
              <div className="container mx-auto px-6">
                <motion.div className="text-center max-w-3xl mx-auto mb-16" variants={sectionAnimation}><h2 className="text-4xl font-bold text-white">Explore Our AI Solutions</h2><p className="text-lg text-gray-400 mt-4">We leverage advanced AI to monitor and protect the core elements of our world.</p></motion.div>
                <div className="grid md:grid-cols-3 gap-8">
                    <a href="/air" className="portal-card"><video autoPlay loop muted playsInline className="portal-video"><source src="/videos/air-background.mp4" type="video/mp4" /></video><div className="portal-content"><div className="content-default"><Wind size={40} /><h3 className="text-4xl font-bold mt-4">Air</h3></div><div className="content-hover"><h3 className="text-2xl font-bold">Air</h3><p className="mt-2 text-sm text-gray-200">Real-time air quality monitoring, pollution source tracking, and predictive weather modeling.</p></div></div></a>
                    <a href="/water" className="portal-card"><video autoPlay loop muted playsInline className="portal-video"><source src="/videos/water-background.mp4" type="video/mp4" /></video><div className="portal-content"><div className="content-default"><Droplets size={40} /><h3 className="text-4xl font-bold mt-4">Water</h3></div><div className="content-hover"><h3 className="text-2xl font-bold">Water</h3><p className="mt-2 text-sm text-gray-200">AI-powered flood & drought forecasting, water quality analysis, and smart resource management.</p></div></div></a>
                    <a href="/land" className="portal-card"><video autoPlay loop muted playsInline className="portal-video"><source src="/videos/land-background.mp4" type="video/mp4" /></video><div className="portal-content"><div className="content-default"><Mountain size={40} /><h3 className="text-4xl font-bold mt-4">Land</h3></div><div className="content-hover"><h3 className="text-2xl font-bold">Land</h3><p className="mt-2 text-sm text-gray-200">Automated deforestation detection, sustainable agriculture insights, and crop disease diagnosis.</p></div></div></a>
                </div>
              </div>
            </motion.section>
            
            {/* --- Eco-Verify Section --- */}
            <motion.section className="py-24 bg-black/20" variants={sectionAnimation} initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.2 }}>
                <div className="container mx-auto px-6">
                    <EcoVerify />
                </div>
            </motion.section>

            {/* --- NEW: Climate Impact Simulator Section (Placed above Unseen Crisis) --- */}
            <motion.section className="py-24" variants={sectionAnimation} initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.2 }}>
                <div className="container mx-auto px-6">
                    <motion.div className="text-center max-w-3xl mx-auto mb-16" variants={sectionAnimation}><h2 className="text-4xl font-bold text-white">Climate Impact Simulator</h2><p className="text-lg text-gray-400 mt-4">See the future. Understand how small changes in global temperature and deforestation can have massive consequences. This demonstrates our predictive modeling capabilities.</p></motion.div>
            ...
            <motion.section className="py-24" variants={sectionAnimation} initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.2 }}>
               <div className="container mx-auto px-6">
                <motion.div className="max-w-4xl mx-auto text-center mb-16" variants={sectionAnimation}><h2 className="text-4xl font-bold text-white">The Unseen Crisis</h2><p className="text-lg text-gray-400">Our planet is facing unprecedented challenges. Technology is our greatest ally in understanding and combating them.</p></motion.div>
                <div className="bg-[#161B22] p-8 md:p-12 rounded-2xl shadow-xl max-w-5xl mx-auto text-left text-gray-300 space-y-4 leading-relaxed border border-gray-700">
                   <p>From melting polar ice caps...</p>
                   <p>Pollution in our air and water...</p>
                   <p>At Bit-Climate, we believe...</p>
                </div>
               </div>
            </motion.section>
            
            <motion.footer className="bg-gray-900 text-gray-400 py-12 mt-16" initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} transition={{ duration: 1.0 }}>
              <div className="container mx-auto px-6 text-center"><p>&copy; 2025 Bit-Climate. All Rights Reserved. Building a resilient planet with AI.</p></div>
            </motion.footer>
        </motion.div>
      </main>
    </>
